<x-layouts.app :title="$genre->name . ' Games'">
    <div class="px-6 py-8 max-w-7xl mx-auto">
        <div class="mb-6">
            ...
            <p class="text-gray-400">{{ $games->total() }} game{{ $games->total() !== 1 ? 's' : '' }} found</p>
        </div>
        ...
        <div class="bg-gray-800 rounded-xl p-6 mb-8">
            <form method="GET" action="{{ route('genres.show', $genre) }}" class="flex flex-col md:flex-row gap-4">
                <!-- Search Input -->
                <div class="flex-1">
                    <label for="search" class="block text-sm font-medium text-gray-300 mb-2">Search in {{ $genre->name }}</label>
                    <input type="text" 
                           id="search" 
                           name="search" 
                           value="{{ $search }}"
                            ...
                            <div class="p-4">
                                ...
                                <div class="flex flex-wrap gap-1">
                                    @foreach($game->genres->where('id', '!=', $genre->id)->take(2) as $otherGenre)
                                        <span class="bg-gray-700 px-2 py-1 rounded text-xs">{{ $otherGenre->name }}</span>
                                    @endforeach
                                </div>
                                ...
                                <div class="mt-2 flex flex-wrap gap-1">
                                    ...
                                    @if($game->platforms->count() > 3)
                                        <span class="text-gray-400 text-xs px-2 py-1">+{{ $game->platforms->count() - 3 }} more</span>
                                    @endif
                                </div>
                            </div>
                        </a>
