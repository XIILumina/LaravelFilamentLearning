<x-layouts.app :title="$genre->name">
    <div class="min-h-screen bg-zinc-950">
        <!-- Header -->
        <div class="px-4 sm:px-6 py-6 border-b border-zinc-800">
            <div class="max-w-5xl mx-auto">
                <a href="{{ route('genres.index') }}" class="inline-flex items-center gap-2 text-zinc-400 hover:text-white text-sm mb-4 transition">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                    </svg>
                    All Genres
                </a>
                <h1 class="text-2xl sm:text-3xl font-bold text-white">{{ $genre->name }}</h1>
                <p class="text-zinc-500 text-sm mt-1">{{ $games->total() }} game{{ $games->total() !== 1 ? 's' : '' }} in this genre</p>
            </div>
        </div>
        ...
        <div class="px-4 sm:px-6 py-4 border-b border-zinc-800 bg-zinc-900/50">
            <div class="max-w-5xl mx-auto">
                <form method="GET" action="{{ route('genres.show', $genre) }}" class="flex flex-col sm:flex-row gap-3">
                ...
                @if($search ?? false)
                    <div class="mt-3 flex items-center justify-between">
                        ...
                        <a href="{{ route('genres.show', $genre) }}" class="text-sm text-orange-500 hover:text-orange-400">Clear ×</a>
                    </div>
                @endif
            </div>
        </div>
            ...
            <div class="max-w-5xl mx-auto">
                ...
                @else
                    <div class="text-center py-16">
                        ...
                        <p class="text-zinc-500 text-sm mb-6">No games match your search in this genre</p>
                        <a href="{{ route('genres.show', $genre) }}" class="inline-flex items-center gap-2 bg-orange-500 hover:bg-orange-600 px-4 py-2 rounded-xl text-white font-medium text-sm transition">
